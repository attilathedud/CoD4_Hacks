...
// File pointer to our text file that has the previous game path.
FILE *gameInfo = NULL;
    ...
    switch(msg)
    {
		...
		*/
        ...
        case WM_COMMAND:
            ...
            {
				// On inject, get our current path. Check to see if decafCoffee.txt exists with a path.
				// If not, open a file select dialog for the user to select the target process.
                case 1:
					GetModuleFileName(NULL, path, sizeof(path));
					for(int i = strlen(path); path[i] != '\\'; i--)
						path[i] = 0;
					
					if(gameInfo = fopen(strcat(path, "decafCoffee.txt"), "r"))
					{
						fgets(gamePath, 256, gameInfo);
						fclose(gameInfo);
					}
					else
					{
						...
						{
							gameInfo = fopen(path, "w");
							if(gameInfo)
							{
								fprintf(gameInfo, "%s", gamePath);
								fclose(gameInfo);
							}
						}
					}
		...
		case WM_TIMER:
			...
			if(nfo == false)
				DrawText(dc, "  = decafCoffee :: Call Of Duty 4 =\n   Code.....................attilathedud\n   Graphics......................iHakthu\n   Tune.............................A-Move", 160, &rect, DT_NOCLIP);
			else
				DrawText(dc, "                            DoxCoding.com presents:\n\n                        decafCoffee.dll by attilathedud\n----------------------------------------------------------------------------------------------------\n Thanks to:                                     Keys:\n         atom0s - for his                         F3 - Toggle Menu        \n         tunnel code and                         Down - Toggle Position\n         his constant help.                      Right - Toggle Option  \n----------------------------------------------------------------------------------------------------\n\n Shout-Outs: \n    pandas, STN, King_Orgy, deadnesser, Sun, nh2, KsBunker,\n    Couch, Muted", 676, &rect2, DT_NOCLIP);
...
int __stdcall WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
    ...
    hwnd = CreateWindow(wc.lpszClassName, "decafCoffee <3", WS_POPUP,  (GetSystemMetrics(SM_CXSCREEN)-400)/2, (GetSystemMetrics(SM_CYSCREEN)-303)/2, 400, 303, NULL, NULL, hInstance, NULL);
